...
下記のReactアプリケーションをコピーして、Claude.aiの「アーティファクト」として実行すると、ブラウザ上で直接翻刻できます。

## 使い方

1. 下のコードをコピー
2. Claude.aiで「Artifact」ボタンをクリック
3. 「React」を選択して貼り付け
4. 「このページを翻刻」ボタンをクリック
5. Claude APIが翻刻結果を生成

## 必要なもの

- Claude.ai のアカウント
- Anthropic API キー（Claude.ai 内で自動的に処理されます）

## アプリコード

以下をReactアーティファクトとして実行してください。
...
翻刻後のテキストのみを出力してください。`
  ...
  const downloadAll = () => {
    ...
    a.href = url;
    a.download = `${manifestData.title}_翻刻.txt`;
